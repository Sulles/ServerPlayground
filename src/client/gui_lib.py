...
class ListObj:
    ...
    def __init__(self, list_of_entries, screen_size, width=300, height=50, x_offset=0, y_offset=0):
        ...
        self.objects = []
        self.font = pygame_font.SysFont('Helvetica', 10)
...
class ErrorObj(PygameObj):
    def __init__(self, text, screen_size):
        ...
        PygameObj.__init__(self, center,
                           ...
                           text=text,
                           font=pygame_font.SysFont('Helvetica', 10))
...
class DisplayBox(PygameObj):
    ...
    def handle_action(self, action_type, action_data=None, mouse_pos=None, key=None):
        if action_type == 'DISPLAY_TEXT':
            print(f'Display box adding text: {action_data}')
            self.add_text(action_data)
        elif action_type == 'LEFT_MOUSE_DOWN':
...
class StackingBoxes:
    def __init__(self, bottom_left, width, height=30, border=True):
        """
        Constructor for multiple display boxes that stack starting at bottom left, and appending new boxes below
        older boxes, which gives the impression of a terminal.
        """
        self.name = 'StackingBoxes'
        self.all_boxes = list()
        self.bottom_left = bottom_left
        self.width = width
        self.height = height
        self.border = border
        self.init_middle_left = self.bottom_left
        self.init_middle_left['height'] -= int(height/2)

        # bottom_left['height'] is assumed to be screen_size['height']
        self.max_boxes = int(bottom_left['height'] / height) + 1

    def add_box(self, text):
        """ Add new box with text... """
        for _ in text.split('\n'):
            print('Creating new box...')
            self.all_boxes.append(DisplayBox(_, [self.init_middle_left['width'], self.init_middle_left['height']],
                                             width=self.width, height=self.height, border=self.border))
            self.refresh_prev_boxes()

    def refresh_prev_boxes(self):
        print('Refreshing previous box heights...')
        for _ in range(1, len(self.all_boxes)):
            self.all_boxes[_].shift_center([0, -self.height])

    def handle_action(self, action_type, action_data=None, mouse_pos=None, key=None):
        if action_type == 'ADD_TEXT':
            print(f'Adding new box with text: {action_data}')
            self.add_box(action_data)

    def draw(self, surface):
        [_.draw(surface) for _ in self.all_boxes]
...
class InputBox(PygameObj):
    def __init__(self, center, width=30, height=30, color_map=None, font=None, font_to_pixel=None):
        ...
        self.font = pygame_font.SysFont('Helvetica', 10) if font is None else font
        PygameObj.__init__(self, center, width, height, color_map,
                           [{'type': 'rect',
                             'color': None,
                             'settings': {
...
class ModifyTextBox:
    def __init__(self, prompt, center, width=None, height=50, font=None):
        """
        Constructor for the ModifyTextBox object, combines a DisplayBox, InputBox, and CheckBox
